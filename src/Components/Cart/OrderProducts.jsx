...
function OrderProducts({ cart, findProductByVariantId, handleQuantityChange, handleRemoveItem, updatingLineId }) {
  return (
    ...
    {cart.lines.edges.map((edge) => {
      ...
      // Get variant details
      const variantTitle = lineItem.merchandise?.title || variant?.title || 'N/A';
      const productTitle = product?.title || lineItem.merchandise?.title || 'Product';
      const price = parseFloat(lineItem.merchandise?.priceV2?.amount || 0);
      const quantity = lineItem.quantity || 0;
      const lineTotal = price * quantity;

      // Check stock availability
      // If product/variant not found in Redux, allow quantity changes (cart item is valid)
      // Only restrict if we have actual stock data
      const stockQuantity = variant?.inventory_quantity !== undefined
        ? variant.inventory_quantity
        : null; // null means we don't have stock info, so allow updates
      // remainingStock = stock left after considering current cart quantity
      const remainingStock = stockQuantity === null ? null : Math.max(0, stockQuantity - quantity);
        ...
        >
          ...
          <div className="flex justify-center md:justify-start">
            {productImage ? (
              ...
              <div
                className="w-32 h-32 flex items-center justify-center rounded-md"
                style={{
                  backgroundColor: theme.colors.border.light,
                  border: `1px solid ${theme.colors.border.light}`,
                }}
              >
                <span style={{ color: theme.colors.text.secondary }}>No Image</span>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col justify-between md:text-right">
            <div>
              ...
              <p className="text-lg font-bold" style={{ color: theme.colors.text.primary }}>
                ₹{lineTotal.toFixed(2)}
              </p>
            </div>
          </div>
